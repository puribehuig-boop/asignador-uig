...
export async function POST(req: Request) {
  try {
    ...
    if (parsed.errors?.length) {
      // No abortamos; continuamos con filas válidas
      console.warn("Errores de parseo:", parsed.errors.slice(0, 3));
    }
    ...
    // Deduplicar por claves
    const studentByCode = new Map<string, { code: string; name: string | null }>();
    ...
    for (const r of rows) {
      if (!studentByCode.has(r.student_code)) {
        studentByCode.set(r.student_code, { code: r.student_code, name: r.student_name ?? null });
      } else {
        // Si llega un nombre nuevo no vacío, actualiza
    ...
    const eligPairs = Array.from(eligSet.values()).map((k) => {
      const [s, c] = k.split("|");
      return { student_code: s, course_code: c };
    });

    // 1) UPSERT students por code
    ...
    // Asegurar que tenemos todos (si el upsert no devolviera todos, re-consulta)
    if (studentRows.length < students.length) {
      const { data, error } = await supabaseAdmin.from("students").select("id, code").in(
        "code",
    ...
    if (courseRows.length < courses.length) {
      const { data, error } = await supabaseAdmin.from("courses").select("id, code").in(
        "code",
    ...
    // 3) Inserta elegibilidades (upsert por (student_id,course_id))
    const eligRows = eligPairs
      .map(({ student_code, course_code }) => {
